from core.data_provider import mnist, kth_action, bair, custom_dataset

datasets_map = {
    'mnist': mnist,
    'action': kth_action,
    'bair': bair,
    'disaster_dataset': custom_dataset  # ✅ Add disaster dataset
}
...
def data_provider(dataset_name, train_data_paths, valid_data_paths, batch_size,
    ...
    print(f"Dataset requested: {dataset_name}")
    print(f"Available datasets: {datasets_map.keys()}")
    ...
    if dataset_name == 'disaster_dataset':
        from core.data_provider import custom_dataset
        train_params = {
        ...
        print("Creating train dataset handle...")
        train_input_handle = custom_dataset.DataProcess(train_params).get_train_input_handle()
        print("Creating test dataset handle...")
        test_input_handle = custom_dataset.DataProcess(test_params).get_test_input_handle()

        print(f"Train Handle: {train_input_handle}")
        print(f"Test Handle: {test_input_handle}")
    ...
    if dataset_name == 'bair':
        ...
        if is_training:
            ...
            return test_input_handle
    elif dataset_name == 'disaster_dataset':
        from core.data_provider import custom_dataset
        train_params = {
            'train_data_paths': train_data_paths,
            'batch_size': batch_size,
